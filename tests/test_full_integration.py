...
class TestMCPServerIntegration:
    ...
    def test_mcp_server_complete_integration(self):
        ...
        assert len(terminal_tools) >= 5, f"Expected at least 5 terminal tools, found {len(terminal_tools)}"
...
class TestLangChainAdapterIntegration:
    ...
    def test_langchain_adapter_complete_integration(self, test_config):
        ...
        required_tools = {
            'list_payments', 'retrieve_payment',        # Payment tools
            'list_payouts', 'retrieve_payout',          # Payout tools  
            'list_terminals', 'retrieve_terminal',      # Terminal tools (new)
            'list_disputes', 'retrieve_dispute',        # Dispute tools
            'list_checkouts', 'retrieve_checkout'       # Checkout tools
        }
        ...
        expected_terminal_tools = {'list_terminals', 'retrieve_terminal', 'update_terminal', 'get_terminal_status', 'identify_terminal'}
