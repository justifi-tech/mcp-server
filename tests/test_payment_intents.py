...
class TestCreatePaymentIntent:
    ...
    async def test_create_payment_intent_success(
        self, test_client, mock_token_response, mock_payment_intent_response
    ):
        ...
        assert result["data"][0]["status"] == "requires_confirmation"

    @pytest.mark.asyncio
    async def test_create_payment_intent_production_without_test_key(
    ...
    async def test_create_payment_intent_invalid_capture_method(self, test_client):
        ...
        with pytest.raises(
            ValidationError, match="capture_method must be 'automatic' or 'manual'"
        ):
            await create_payment_intent(
                client=test_client,
                amount=1000,
    ...
    async def test_create_payment_intent_invalid_confirmation_method(self, test_client):
        ...
        with pytest.raises(
            ValidationError, match="confirmation_method must be 'automatic' or 'manual'"
        ):
            await create_payment_intent(
                client=test_client,
                amount=1000,
...
class TestCapturePaymentIntent:
    ...
    @pytest.mark.asyncio
    @respx.mock
    async def test_capture_payment_intent_success(
        self, test_client, mock_token_response, mock_captured_intent_response
    ):
    ...
    @pytest.mark.asyncio
    async def test_capture_payment_intent_production_without_test_key(
        self, production_client
    ):
...
class TestCancelPaymentIntent:
    ...
    async def test_cancel_payment_intent_success(
        self, test_client, mock_token_response, mock_cancelled_intent_response
    ):
        ...
        assert result["data"][0]["id"] == "pi_test123"
        assert result["data"][0]["status"] == "canceled"

    @pytest.mark.asyncio
    async def test_cancel_payment_intent_production_without_test_key(
        self, production_client
    ):
    ...
    async def test_cancel_payment_intent_invalid_reason(self, test_client):
        ...
        with pytest.raises(
            ValidationError, match="cancellation_reason must be a string"
        ):
            await cancel_payment_intent(
                client=test_client,
                intent_id="pi_test123",
...
class TestConfirmPaymentIntent:
    ...
    @pytest.mark.asyncio
    @respx.mock
    async def test_confirm_payment_intent_success(
        self, test_client, mock_token_response, mock_payment_intent_response
    ):
    ...
    @pytest.mark.asyncio
    async def test_confirm_payment_intent_production_without_test_key(
        self, production_client
    ):
